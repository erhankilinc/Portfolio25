// Project links click handlers (pour les liens dans l'overlay)
    document.querySelectorAll('.project-link').forEach(link => {
        link.addEventListener('click', function(e) {
            ...
            // Pour l'instant, on affiche des notifications car les pages détaillées ne sont pas encore créées
            if (linkText.includes('Voir le rapport')) {
                showDetailedNotification(
                    `📋 Page détaillée de ${saeNumber}`, 
                    `La page complète avec photos, documentation et détails du projet sera bientôt disponible. En attendant, vous pouvez consulter le résumé dans le modal ci-dessus.`,
                    'info'
                );
            } else if (linkText.includes('Configuration') || linkText.includes('Code source')) {
                showDetailedNotification(
                    `💻 Fichiers techniques de ${saeNumber}`,
                    `Les configurations, scripts et fichiers techniques seront disponibles dans la page détaillée du projet. Cliquez sur la carte pour voir plus d'informations.`,
                    'info'
                );
            } else if (linkText.includes('Schémas réseau') || linkText.includes('Topologie')) {
                showDetailedNotification(
                    `🌐 Documentation réseau`,
                    `Les schémas réseau et topologies seront visibles dans la galerie du projet détaillé.`,
                    'info'
                );
            } else if (linkText.includes('Mesures pratiques') || linkText.includes('Analyses données')) {
                showDetailedNotification(
                    `📊 Données et résultats`,
                    `Les mesures, analyses et graphiques seront disponibles dans la section "Résultats" du projet détaillé.`,
                    'info'
                );
            } else if (linkText.includes('Solution déployée') || linkText.includes('Projet final')) {
                showDetailedNotification(
                    `🎯 Livrables du projet`,
                    `La solution complète et tous les livrables seront présentés dans la page détaillée avec captures d'écran et documentation.`,
                    'success'
                );
            } else if (linkText.includes('Guide sécurité') || linkText.includes('Politique sécurité')) {
                showDetailedNotification(
                    `🔒 Documents de sécurité`,
                    `Les documents de sécurité sont confidentiels mais des extraits seront disponibles dans la page détaillée.`,
                    'warning'
                );
            } else if (linkText.includes('Voir le site')) {
                showDetailedNotification(
                    `🌍 Site web SAE 1.4`,
                    `Ce portfolio EST le résultat de votre SAE 1.4 ! La page détaillée montrera le processus de création.`,
                    'success'
                );
            } else {
                showDetailedNotification(
                    `📁 Document "${linkText}"`,
                    `Ce document sera accessible dans la page détaillée du projet avec aperçu et possibilité de téléchargement.`,
                    'info'
                );
            }
            
            // Suggestion pour créer les pages
            setTimeout(() => {
                showDetailedNotification(
                    `💡 Conseil de développement`,
                    `Pour créer la page détaillée : copiez le template "sae-detail.html", personnalisez le contenu et ajoutez vos photos !`,
                    'info'
                );
            }, 3000);
        });
    ...
    // Fonction pour notifications détaillées
    function showDetailedNotification(title, message, type = 'info') {
        ...
        if (type === 'info') bgColor = 'linear-gradient(135deg, #00b4d8, #0077b6)';
        ...
        }, 8000); // Plus long pour lire le message
